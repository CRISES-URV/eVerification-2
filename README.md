...
### Verification (any SC with stored share and share commitment can performs this)

		Step 1 from shares generation process are repeated here.
		3. Once the receiver SC has stored their own share and share commitment, it must verifies the correctness generation made by generator SC.
			- Share verification: CLA=80, INS=A0, P1-P2=00.
			- Share commitment verification: CLA=80, INS=B0, P1-P2=00.

### Encryption (any SC can performs this)

		Steps 1 and 2 (if it is not performed before) from shares generation process are repeated here.
		2. The data to encrypt are sent to SC and it is encrypted with APDU: CLA=90, INS=0C, P1-P2=00, Lc=20, Data=message to encrypt.
		3. ElGamal encryption result is recovered sending the APDU: CLA=90, INS=0D, P1-P2=00.


### Homomorphic Decryption (tally)

		1. We have to generate evaluation values and Lagrange coefficients in the client side.
		Steps 1 and 2 from shares generation process are repeated here.
